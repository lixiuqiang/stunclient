...
#定义产生transaction id的函数
import os
if hasattr(os, 'urandom'):
    def getRandomTID():
        return os.urandom(16)#os.urandom(n)是产生n直接的随机字符串
...
#                                             for host, port in servers]
        self.servers = [(host, port) for host, port in servers]#得到server列表
        # </AP>
    ...
    def initialStunRequest(self, address):
        ...
        tid = getRandomTID()
        #添加重传函数，在超时后继续发送请求
        delayed = reactor.callLater(INITIAL_TIMEOUT, self.retransmitInitial, address, tid)
        #设置消息的状态为delayed, 并添加到已发送队列中，然后发送消息
...
if __name__ == "__main__":
    ...
    class TestStunDiscoveryProtocol(StunDiscoveryProtocol):
        ...
        def finishedStun(self):
            #it over write the super's function
    ...
    logFp = open("log.log", "w")
    log.startLogging(logFp)
    try:
        reactor.listenUDP(5061, stunClient)
        
    except:
        print "port 5061 is been using!"
